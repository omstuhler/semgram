...
#' @param parse_multi_token_entities Should multi-token entities (e.g. "Harry Potter') be considered. Defaults to TRUE. When using multi-token entities, it is crucial that tokens are separated by a space character. Make sure that your input matches the tokenized version in your tokens object.
#' For instance, hyphens are usually considered a token in tokenization, so that "Claude Levi-Strauss" should be passed to entities as "Claude Levi - Strauss".
#' @param extract Parameter defines whether extracted motifs are represented in "lemma" or "token" form. Defaults to "lemma" which reduces sparsity and is preferable for most purposes.
...
#' @param get_aux_verbs Should auxiliary verbs (e.g. can, could, may, must) be considered a actions? Defaults to FALSE.
